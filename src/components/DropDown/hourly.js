            ...
            <MDBContainer className="d-flex flex-row hourlyContainer" style={{ margin: "0 0" }}> {/*same idea as daily component (see daily.js) but not title and passing throught hour not day */}
                {
                    this.props.data.map(data => {
                        ...
                        return (
                            <Card time={hours} temp={temp} icon={data.weatherIcon} condition={data.weatherDesc} celsius={this.props.celsius} wind={data.wind}/>/*explained more in daily.js, here we are passing through the hour */
                        )
                    })
                }
            </MDBContainer>
