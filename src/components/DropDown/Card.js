    ...
    render() {
        return (
            <MDBContainer className="card"> {/*although the classname is hourly, this was because originally for just hourly but was then abstracted to reuse for daily too, thats why component was renamed to just card */}
                ...
                <p>{this.props.wind.speed.value}{this.props.kmh?"km/h":"mph"}</p>
                <h2 style={{fontWeight:"400"}}>{this.props.temp}&#176;{this.props.fahrenheit ? "F" : "C"}</h2> {/*checks if in celcius or farenheight, displays appropriate symbol */}
